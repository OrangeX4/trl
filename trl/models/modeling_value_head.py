...
from transformers import AutoModelForCausalLM, AutoModelForSeq2SeqLM, AutoModelForVision2Seq
...
class AutoModelForVison2SeqLMWithValueHead(PreTrainedModelWrapper):
    r"""
    A seq2seq model with a value head in addition to the language model head.
    This class inherits from `~trl.PreTrainedModelWrapper` and wraps a
    `transformers.PreTrainedModel` class. The wrapper class supports classic functions
    ...
    transformers_parent_class = AutoModelForVision2Seq
    lm_head_namings = ["lm_head", "embed_out", "output_projection"]
    supported_args = (
        "summary_dropout_prob",
        "v_head_initializer_range",
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        v_head_kwargs, _, _ = self._split_kwargs(kwargs)
        self.is_encoder_decoder = True
        self.is_vision_encoder_decoder = True
